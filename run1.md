...
The Login and Register Google buttons use Google Identity Services plus
`supabase.auth.signInWithIdToken`, so the direct button flow does not use a
redirect callback. If Google still shows `Error 400: redirect_uri_mismatch`,
the account-security "Connect Google sign-in" flow or an older deployed build
is using redirect OAuth. Register the exact active Supabase callback in Google
Cloud, ensure the production build contains the current direct-token flow,
and make sure `supabase/config.toml`, `VITE_SUPABASE_URL`, Supabase Google
provider settings, and CLI deployments all use the same project.
