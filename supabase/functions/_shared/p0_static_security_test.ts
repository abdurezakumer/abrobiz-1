...
Deno.test('Auth OTPs are generated by Supabase Auth and delivered by the AbroBiz mailer', async () => {
  ...
  assert.match(signup, /sendAuthOtpEmail/)
  ...
  assert.match(verify, /type: 'magiclink'/)
  assert.match(verify, /\\^\\\\d\\{6\\}\\$/)
  assert.match(resend, /auth\.admin\.generateLink/)
  assert.match(resend, /sendAuthOtpEmail/)
