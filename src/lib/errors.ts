export function friendlyError(error: unknown): string {
  ...
  if (/browser could not (prepare|read) the image|could not prepare the image|could not read the selected photo|cannot read the selected photo/i.test(msg)) return 'This phone photo could not be read. Choose a JPG or PNG photo and try again.'
  ...
  if (/upload (network connection|timed out|was interrupted)|network.*upload|upload.*interrupted|upload.*timeout|failed to fetch|load failed|networkerror|network request failed|connection.*failed/i.test(msg)) return 'The upload was interrupted. AbroBiz retried it safely—please tap the receipt area to try once more.'
  ...
  if (/valid active referral code|referral code/i.test(msg)) return 'That referral code is not active. Check the code and try again, or choose Skip.'
  if (/valid phone number|phone number/i.test(msg)) return 'Enter a valid phone number and try again.'
  if (/platform owner must remain a super administrator/i.test(msg)) return 'The AbroBiz platform owner is permanently protected as a Super Admin.'
